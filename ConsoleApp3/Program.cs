    ...
    static int GetUserIdFromDatabase(SqlConnection connection, string fio)
    {
        ...
        using (SqlCommand command = new SqlCommand(query, connection))
        {
            ...
            else
            {
                ...
                return 1; // Или любое другое значение, которое удобно использовать по умолчанию
            }
        }
    }
    ...
    static int GetProductIdFromDatabase(SqlConnection connection, string productName)
    {
        ...
        using (SqlCommand command = new SqlCommand(query, connection))
        {
            ...
            else
            {
                ...
                return 1; // Или любое другое значение, которое удобно использовать по умолочанию
            }
        }
    }
>>>>>>> Изменено
